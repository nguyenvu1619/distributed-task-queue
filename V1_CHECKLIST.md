...
- [ ] **Counter reconciler.** Nothing recomputes `queue_shards.running` /
      ...
      reaper, next to the release CTEs in `recoverJobs`.
- [x] **[obsolete] Double `ROLLBACK`** on settle-after-lease-loss — fixed structurally: the
      settle paths are single statements now, so there is no transaction to roll back at all,
      let alone twice.
...
- [ ] `priority` — change pull ordering to `(queue_id, priority, id)`; `created_at` ties are
      ...
      order on bare `created_at` (`pullJobFast` / `pullJobWithCoordination`).
...
- [ ] **A grouped job with no `group_queue_limits` row stalls the entire queue.** The coordination
      pull selects the single oldest PENDING row and refuses the pull if that row's group cannot
      take a slot (the `admitted` gate in `pullJobWithCoordination` matches zero rows, so nothing
      downstream writes) — so an undeliverable grouped
