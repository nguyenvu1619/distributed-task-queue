...
| 03 | [03-storage.md](03-storage.md) | **sharded pull, lease fencing, dual-path settle** | 619+ / 207− |
...
| 08 | [08-observability.md](08-observability.md) | logger, Executor, pool config | 133+ / 38− |
| 09 | [09-publish.md](09-publish.md) | **transactional + batched + dedup publish** | 224+ / 49− |
| 10 | [10-queue-resolution.md](10-queue-resolution.md) | `ensureQueue`, drift warning | 98+ / 11− |
| 11 | [11-worker-hardening.md](11-worker-hardening.md) | poison discard, bounded shutdown | 358+ / 40− |
| 12 | [12-listen-notify.md](12-listen-notify.md) | `PgNotifier` + `pg_notify` | 382+ / 7− |
...
- **PR03** — `isExpired` is always `false` (the SELECT already filters to
  `PENDING`), so both `!isExpired` guards are dead code.
...
- PRs 01–07 reproduce the old `936d613` byte-for-byte (verified).
- PR16's tree is identical to the working tree this was split from (verified by
  tree hash).
- Every one of the 16 stages typechecks on its own.
