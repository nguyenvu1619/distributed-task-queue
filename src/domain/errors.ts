...
export const ErrorCodes = {
  ...
  /** An argument this library rejected before touching the database. See {@link InvalidInputError}. */
...
/**
 * An argument this library refused before it reached the database: an unparseable
 * duration, a payload the serializer cannot represent, a publish to a queue that
 * requires a group id without one, or a queue redefined with a different config.
 *
 * Always a bug in the calling code rather than a transient condition, so the fix is
 * to correct the call, never to retry it.
 */
export class InvalidInputError extends TaskQueueError {
  constructor(message: string, context: ErrorContext = {}) {
    super(ErrorCodes.INVALID_INPUT, false, message, context);
  }
}
