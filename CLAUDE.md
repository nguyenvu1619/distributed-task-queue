...
- **Coordination path** (`pullJobWithCoordination`): a single multi-CTE statement that also updates `queue_shards` and `group_queue_limits` counters — used when concurrency > 0 or `requiresGroupId = true`. One round trip, like the fast path; atomicity comes from the statement itself, not BEGIN/COMMIT.
