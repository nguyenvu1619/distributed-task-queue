...
- The **coordination path** — taken whenever `concurrency > 0` or
  `requiresGroupId` is set — is a transaction of roughly seven statements per
  job: claim a shard, `SELECT … FOR UPDATE SKIP LOCKED` the next job, bump the
  group counter, bump the shard counter, write the lease, commit. That is
  ~3x slower, and ~9x with groups on top.
